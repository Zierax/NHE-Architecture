...
|---|---|---|---|
| `N_LAYERS = 26` (`runtime_rollback.py:20`) | 26 layers x hidden 1152 x vocab 262k | Qwen0.5B 24x896x151936, Qwen1.5B 28x1536x151936 | `MODEL_REGISTRY` dict + dynamic inference `get_n_layers(model, fallback)` and `get_hidden_size()` from `model.config` after load (`runtime_rollback_qwen.py:70-90`) |
...
| `dtype=torch.float16` (`runtime_rollback.py:34`) | Works for Gemma | Qwen config dtype `bfloat16`; on CPU float16 slower than float32 (STATUS notes 15x) | `model_and_tok` uses float16 only if CUDA available else float32, with `cache_dir` explicitly set to `D:/hf_cache` (`runtime_rollback_qwen.py:98-115`) |
...
## 7. REAL Qwen2.5-0.5B results (2026-09-05, supersedes Sec 1.4/Sec 2 for 0.5B)
...
hall tin {5,7,7,8,8,8,9,9,10}, truth tin {5..10} overlapping. **There is no
