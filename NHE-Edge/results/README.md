...
- `bench_hard.json` — 99 items: all 54 `africa_largest` (16 greedy-wrong) + 15
  ...
  `../bench_build.py`. Baseline strict 0.596.
- `bench_random.json` — 99 random items from the same 361 pool, seed 42, overlap
  19 with hard. Built by `../bench_random_build.py` (byte-identical). Baseline 0.099.
...
- `greedy_flows_africa.npz` (58 MB, tracked) — real Gemma flows for the detector.
...
## Naming
`none` = detector records but never cuts. `mask` = soft ×0.3 on fire (w≤5, t90).
`abstain` = refuse on fire. `_s1000…` = sampled seed. No suffix = greedy.
