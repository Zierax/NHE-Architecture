"""AtP causal attribution for Qwen2.5-0.5B (port of attribute_causal2.py).

- Qwen ChatML prompt instead of Gemma turns.
- Hallucinations from the Qwen Africa none run (9 greedy-wrong).
- Mid band scaled to 24 layers (default 10-20, covers signal layers 16-22).
- fp16 load (memory), float32 for backward (matches Gemma pipeline).
- Output: results/attribution_causal2_africa_qwen2.5-0.5b.json
"""
import json
import os
import sys
...
BASELINE_FILE = os.path.join(RES_DIR, f"eval_runtime_africa_{MODEL_KEY}_jump_gt_L22_t90_none_w10.json")
PROGRESS = os.path.join(RES_DIR, f"attribution_causal2_progress_{MODEL_KEY}.npz")
OUT_FILE = os.path.join(RES_DIR, f"attribution_causal2_africa_{MODEL_KEY}.json")
...
def main():
    ...
    print(f"qwen greedy-hallucinated examples: {len(hall)}", flush=True)
    assert len(hall) > 0, "no hallucinations to attribute"

    t0 = time.time()
    model, tok = qq.model_and_tok(MODEL_KEY)
    model = model.float()
    ...
    print(f"n_layers={n_layers} n_units={n_units}", flush=True)
    ...
    for r in hall:
        ...
        q = r["question"]
        ans = r["answer"]
        ids = qq.build_prompt_ids(tok, q, MODEL_KEY)
        gen_ids = tok(r["generated"], add_special_tokens=False)["input_ids"]
        ans_ids = tok.encode(ans, add_special_tokens=False)
        if gen_ids is None or ans_ids is None or len(gen_ids) == 0 or len(ans_ids) == 0:
    ...
    def top_where(cond, k=TOP_K):
        sub = [e for e in flat if cond(e)]
        return [{"layer": l, "unit": u, "causal": s, "c_wrong": cw, "c_correct": cc} for l, u, s, cw, cc in sub[:k]]

    mid = top_where(lambda e: MID_LO <= e[0] <= MID_HI)
    wrong_only = top_where(lambda e: e[3] > 0 and e[4] <= 0.0)
    mid_wrong_only = top_where(lambda e: MID_LO <= e[0] <= MID_HI and e[3] > 0 and e[4] <= 0.0)
    out = {"method": "atp_two_targets_fp32", "model": MODEL_KEY, "n_examples": n_done,
           "mid_band": [MID_LO, MID_HI], "top_by_causal": top_all, "top_mid": mid,
           "top_wrong_only": wrong_only, "top_mid_wrong_only": mid_wrong_only}
    with open(OUT_FILE, "w", encoding="utf-8") as fh:
        json.dump(out, fh, indent=1)
    for name, lst in [("ALL", top_all), (f"MID {MID_LO}-{MID_HI}", mid), ("WRONG_ONLY", wrong_only), ("MID+WRONG", mid_wrong_only)]:
