import json
import sys

sys.stdout.reconfigure(encoding="utf-8", errors="replace")

base = {r["question"]: r for r in json.load(open("results/eval_africa_baseline.json", encoding="utf-8"))["results"]}
for tag in ["jump_gt_L18_t95_mask", "jump_gt_L18_t90_mask"]:
    ev = json.load(open(f"results/eval_runtime_africa_{tag}.json", encoding="utf-8"))
    print(f"=== {tag}: fired={ev['n_fired']}/{ev['n']} hall={ev['hallucination_rate']}")
    for r in ev["results"]:
        b = base[r["question"]]
        fired = r["fired_at"] is not None
        flip = r["correct"] != b["correct"]
        if fired or flip:
            kind = ""
            if r["correct"] != b["correct"]:
                kind = "W2C" if r["correct"] else "C2W"
            mark = "FIRE" if fired else "    "
            print(f"  {mark} at={r['fired_at']} {kind:<3} {r['question']}")
            print(f"      base: {b['generated'][:55]!r}")
            print(f"      rt  : {r['generated'][:55]!r}")